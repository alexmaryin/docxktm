...
class TableTest {
    ...
    fun `Create nested table inside cell`() {
        DocxNew(Paths.TEST_DOCX_DIR + "nested_table.docx") {
            body {
                paragraph {
                    text("Nested table example", normalTextStyle.copy(size = 28, align = JcEnumeration.CENTER))
                }
                table(TableStyle(borders = TableBorder.OnlyMargin(BorderStyle(width = 1)))) {
                    row {
                        ...
                        cell {
                            ...
                            table(
                                TableStyle(
                                    layoutType = STTblLayoutType.FIXED,
                                    width = TableWidth.Percent(75),
                                    borders = TableBorder.All(
                                        BorderStyle(
                                            type = STBorder.DOT_DASH,
                                            width = 2,
        ...
        DocxNew(Paths.TEST_DOCX_DIR + "table_cells.docx") {
            body {
                table {
                    ...
                    row {
                        cell(
                            CellStyle(spannedCells = 2, verticalAlign = STVerticalJc.CENTER)
                    ...
                    row {
                        ...
                        cell(
                            CellStyle(spannedCells = 2, verticalAlign = STVerticalJc.BOTTOM)
                        ) { paragraph { text("Spanned 2 cells") } }
                    }
                }
            }
